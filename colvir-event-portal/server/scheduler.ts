 ...
 * При нескольких репликах задачи возьмет та, что проснется первой: подбор
 * защищен проверкой статуса цикла, а напоминания — полем reminder_sent_at,
...
export function startScheduler(intervalMs = DEFAULT_INTERVAL_MS): SchedulerHandle {
  ...
  const tick = async () => {
    // Пропускаем такт, если предыдущий еще не закончился.
    if (running) return;
    running = true;
    try {
