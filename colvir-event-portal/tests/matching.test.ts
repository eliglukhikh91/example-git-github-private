...
describe('Подбор пар Random Coffee', () => {
  ...
  test('приоритет у того, у кого меньше вариантов', () => {
    // У «d» единственный возможный партнер — «a». Жадный по числу вариантов
  ...
  test('нечетный участник становится третьим, а не остается без встречи', () => {
    const { plan, unmatched } = buildMatchPlan([
      candidate('a', '10:00'),
      candidate('b', '10:00'),
      candidate('c', '10:00')
...
describe('Разбор времени слота', () => {
  test('берет начало слота по Москве', () => {
