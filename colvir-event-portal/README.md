...
Тема **общая для компании**: её переключает администратор во вкладке
«Оформление» панели управления, остальные сотрудники переключателя не видят —
только результат. Тема хранится на сервере (`app_settings`), клиент забирает её
...
Утверждённые цвета приглушённые и светлые: белый текст на шалфейном и
терракотовом даёт 3.1–3.3:1 при норме WCAG AA 4.5:1 — кнопка «Записаться» была
бы нечитаемой. Поэтому утверждённый оттенок живёт в `--color-accent-decor`
(свотчи в админке, декоративные иконки баннера), а `--color-accent` — тот же
тон, затемнённый до проходящего контраста. Морозный тил и бренд-синий проходят
...
| `--color-accent-decor` | утверждённый цвет темы: свотчи и декор |
...
взяты эквиваленты из уже подключённой lucide-react, вторую иконочную библиотеку
ради трёх глифов не добавляли.
...
вариантов: первым получает пару тот, у кого меньше всего возможных партнёров.
Ранее встречавшиеся пары не повторяются, пока есть альтернатива. При нечётном
...
При нескольких репликах дубликатов не будет: подбор защищён статусом цикла,
напоминания — полем `reminder_sent_at`.

**Уведомления** приходят в портал обоим участникам. Почты и Slack в проекте нет
— для рассылки за пределы портала понадобится SMTP или интеграция с мессенджером.
...
во всём приложении, включая панель администратора. Палитры проверены на
...
подзаголовок. Текст берётся из CMS и делится по первому двоеточию
